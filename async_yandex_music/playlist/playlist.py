...
from async_yandex_music import YandexMusicObject

if TYPE_CHECKING:
    from async_yandex_music import Client, User, Cover, MadeFor, TrackShort, PlaylistAbsence, PlayCounter,\
...
class Playlist(YandexMusicObject):
    ...
    async def rename(self, name: str) -> None:
        ...
        self.__dict__.clear()
        playlist = await client.users_playlists_name(kind, name)
        self.__dict__.update(playlist.__dict__)
    ...
    async def fetch_tracks(self, *args, **kwargs) -> List['TrackShort']:
        ...
        """
        data = await self.client.users_playlists(self.kind, self.owner.uid, *args, **kwargs)
        return data.tracks
    ...
    def de_json(cls, data: dict, client: 'Client') -> Optional['Playlist']:
        ...
        from async_yandex_music import User, MadeFor, Cover, PlayCounter, TrackShort, \
